...
333 accessibility checks against the six live pages every build, plus 28 contrast, 16 reading-level and 497 invariant checks. The build fails on any. Earlier browser passes fixed a claim code that never took focus (shown once, unrecoverable, and the screen reader user was the one person not told), 46 unnamed sections announced as anonymous regions, and a 22px target under WCAG 2.5.8.
...
- 124 Apex tests
- 497 structural invariants (about one second, no org required)
...
All ten auditors are static Python and Node: 497 invariants, accessibility, contrast, component, link and copy, reading level, responsible AI, and a check that our own published numbers have not gone stale. They run in about a second on every push, with no org and no model.
